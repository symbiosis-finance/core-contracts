...
describe("MulticallRouter tests", function () {
    ...
    it("Should check stable 3 swaps", async () => {
        let amountIn = 10000;
        ...
        await multicallRouter.connect(sender).multicall(
            amountIn,
            [stableSwapCalldata, swapCalldata, stableSwapCalldata],
            [stableDexFirst.address, uniRouter.address, stableDexThird.address],
            [firstToken.address, secondToken.address, thirdToken.address, fourthToken.address],
            [100, 36, 100],
            recipient.address
        );

        let expectedBalance = 9016;

        console.log(
            "Final balance:",
            (await fourthToken.balanceOf(recipient.address)).toString()
        );

        expect(await fourthToken.balanceOf(recipient.address)).to.eq(
            expectedBalance
        );
    });
});