...
describe("MulticallRouter tests", function () {
    beforeEach(async () => {
        ...
        fourthToken = await ERC20.deploy("Fourth Token", "FOURTH", 18)
        ...
        stableDexThird = await StableDex.deploy(
            [thirdToken.address, fourthToken.address],
            [18, 18],
            "Symbiosis STBL pool",
            "SymbPool",
            50,
        ...
        await thirdToken.mint(liquidityProvider.address, String(LIQUIDITY_PROVIDER_BALANCE * 2));
        await fourthToken.mint(liquidityProvider.address, String(LIQUIDITY_PROVIDER_BALANCE));
        ...
        expect(await thirdToken.balanceOf(liquidityProvider.address)).to.eq(String(LIQUIDITY_PROVIDER_BALANCE * 2));
        expect(await fourthToken.balanceOf(liquidityProvider.address)).to.eq(String(LIQUIDITY_PROVIDER_BALANCE));
        ...
        await thirdToken.connect(liquidityProvider).approve(stableDexThird.address, ethers.constants.MaxUint256);
        await fourthToken.connect(liquidityProvider).approve(stableDexThird.address, ethers.constants.MaxUint256);
        ...
        expect(await thirdToken.balanceOf(stableDexThird.address)).to.eq(LIQUIDITY_PROVIDER_BALANCE);
        expect(await fourthToken.balanceOf(stableDexThird.address)).to.eq(LIQUIDITY_PROVIDER_BALANCE);
    ...
    it("Should check 2 swaps and addLiquidity", async () => {
        let amountIn = 10000;

        let stableSwapCalldata = stableSwapInterface.encodeFunctionData("swap", [
            0,
            1,
            0,
            0,
        ...
        const addLiquidityIface = new ethers.utils.Interface([
            "function addLiquidity(uint256[],uint256,uint256)"]);

        let addLiquidityCalldata = addLiquidityIface.encodeFunctionData(
            "addLiquidity",
            [
                [0, 0],
                0,
        ...
        let lpTokenAddr = (await stableDexThird.swapStorage()).lpToken;
        const ERC20Abi = require("../abi/ERC20Mock.json");
        const ERC20 = await ethers.getContractFactory(ERC20Abi.abi, ERC20Abi.bytecode);
        let lpToken = ERC20.attach(lpTokenAddr);

        await multicallRouter.connect(sender).multicall(
            amountIn,
            [stableSwapCalldata, swapCalldata, addLiquidityCalldata],
            [stableDexFirst.address, uniRouter.address, stableDexThird.address],
            [firstToken.address, secondToken.address, thirdToken.address, lpToken.address],
            [100, 36, 164],
            recipient.address
        );

        let expectedBalance = 9034;

        console.log(
            "Final balance:",
            (await lpToken.balanceOf(recipient.address)).toString()
        );

        expect(await lpToken.balanceOf(recipient.address)).to.eq(
            expectedBalance
        );
    });
});