...
describe("MetaRouter synth V2 tests (3 tokens in first, final paths)", function () {
	beforeEach(async () => {
		...
		await library.addLiquidity(
			...
			[syntTokenAmount, syntMiddleTokenAmount],
			uniDex,
			uniFactory,
			[
				[sTestToken, secondMiddleToken],
				[secondMiddleToken, thirdToken],
			],
			[
				[syntTokenAmount, secondMiddleTokenAmount],
	...
	it("Should check metaRoute V2 with first manager chain", async () => {
		let amountIn = 5000;
		...
		let firstSwapCalldata = swapInterface.encodeFunctionData(
			...
			[
				amountIn,
				0,
				[thirdToken.address, secondMiddleToken.address, sTestTokenAddr],
				metaRouter.address,
				ethers.constants.MaxUint256,
			]
		);
