...
describe("MetaRouter V2 tests", function () {
	beforeEach(async () => {
		...
		await secondToken.mint(sender.address, 10000);
	...
	it("Should check metaRoute V2 without first swap", async () => {
		let amountIn = 5000;
		...
		let tx = await metaRouter.connect(sender).metaRoute({
			...
			approvedTokens: [secondToken.address],
